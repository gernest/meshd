...
func NewClient(logger logr.Logger, masterURL, kubeConfig string) (Client, error) {
...
func buildConfig(logger logr.Logger, masterURL, kubeConfig string) (*rest.Config, error) {
	if os.Getenv("KUBERNETES_SERVICE_HOST") != "" && os.Getenv("KUBERNETES_SERVICE_PORT") != "" {
		// If these env vars are set, we can build an in-cluster config.
		logger.Info("Creating in-cluster client")
		return rest.InClusterConfig()
	}

	if masterURL != "" || kubeConfig != "" {
		logger.Info("Creating cluster-external client from provided masterURL or kubeconfig")
...
func buildKubernetesClient(logger logr.Logger, config *rest.Config) (*kubernetes.Clientset, error) {
...
func buildSmiAccessClient(logger logr.Logger, config *rest.Config) (*accessclient.Clientset, error) {
	logger.Info("Building SMI Access Client...")
...
func buildSmiSpecsClient(logger logr.Logger, config *rest.Config) (*specsclient.Clientset, error) {
	logger.Info("Building SMI Specs Client...")
...
func buildSmiSplitClient(logger logr.Logger, config *rest.Config) (*splitclient.Clientset, error) {
	logger.Info("Building SMI Split Client...")
