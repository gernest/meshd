...
func TestConfigureCoreDNS(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expCorefile: ".:53 {\n    errors\n    health {\n        lameduck 5s\n    }\n    ready\n    kubernetes {{ pillar['dns_domain'] }} in-addr.arpa ip6.arpa {\n        pods insecure\n        fallthrough in-addr.arpa ip6.arpa\n        ttl 30\n    }\n    prometheus :9153\n    forward . /etc/resolv.conf\n    cache 30\n    loop\n    reload\n    loadbalance\n}\n",
			expCustoms: map[string]string{
				"dream.mesh.server": "#### Begin dream Mesh Block\ndream.mesh:53 {\n    errors\n    cache 30\n    forward . 10.10.10.10:53\n}\n#### End dream Mesh Block\n",
			},
			expRestart: false,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			err := client.ConfigureCoreDNS(ctx, "dream-mesh", "dream-mesh-dns", 53)
			if test.expErr {
				require.Error(t, err)
				return
			}
			...
			restarted := coreDNSDeployment.Spec.Template.Annotations["dream-mesh-hash"] != ""
...
func TestConfigureKubeDNS(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expStubDomains: `{"dream.mesh":["10.10.10.10:53"]}`,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			err := client.ConfigureKubeDNS(ctx, "dream-mesh", "dream-mesh-dns", 53)
			if test.expErr {
				require.Error(t, err)
				return
			}
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			if test.hasCustom {
				...
				_, exists := customCfgMap.Data["dream.mesh.server"]
