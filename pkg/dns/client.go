...
import (
	...
	"github.com/gernest/meshd/pkg/safe"
...
const (
	...
	blockHeader  = "#### Begin dream Mesh Block"
	blockTrailer = "#### End dream Mesh Block"
)
...
type xdep struct {
	get    func(ctx context.Context, ns, nams string) (*appsv1.Deployment, error)
	update func(ctx context.Context, ns string, m *appsv1.Deployment) (*appsv1.Deployment, error)
}
type xmap struct {
	get    func(ctx context.Context, ns, nams string) (*corev1.ConfigMap, error)
...
type xsvc struct {
	get func(ctx context.Context, ns, nams string) (*corev1.Service, error)
}
type xclient struct {
	xdep xdep
	xmap xmap
		...
		kubeClient: xclient{
			xdep: xdep{
				get: func(ctx context.Context, ns, name string) (*appsv1.Deployment, error) {
					return kubeClient.AppsV1().Deployments(ns).Get(ctx, name, metav1.GetOptions{})
				},
				update: func(ctx context.Context, ns string, m *appsv1.Deployment) (*appsv1.Deployment, error) {
					return kubeClient.AppsV1().Deployments(ns).Update(ctx, m, metav1.UpdateOptions{})
				},
			},
			xmap: xmap{
				get: func(ctx context.Context, ns, name string) (*corev1.ConfigMap, error) {
					return kubeClient.CoreV1().ConfigMaps(ns).Get(ctx, name, metav1.GetOptions{})
				},
				update: func(ctx context.Context, ns string, m *corev1.ConfigMap) (*corev1.ConfigMap, error) {
					return kubeClient.CoreV1().ConfigMaps(ns).Update(ctx, m, metav1.UpdateOptions{})
				},
				create: func(ctx context.Context, ns string, m *corev1.ConfigMap) (*corev1.ConfigMap, error) {
					return kubeClient.CoreV1().ConfigMaps(ns).Create(ctx, m, metav1.CreateOptions{})
				},
			},
			xsvc: xsvc{
...
func (c *Client) CheckDNSProvider(ctx context.Context) (Provider, error) {
	c.logger.Info("Detecting DNS provider...")
...
func (c *Client) coreDNSMatch(ctx context.Context) (bool, error) {
	c.logger.Info("Checking if CoreDNS is installed in namespace %q...", metav1.NamespaceSystem)

	dnsDeployment, err := c.kubeClient.xdep.get(ctx, metav1.NamespaceSystem, "coredns")
	if kerrors.IsNotFound(err) {
		c.logger.Info("CoreDNS dnsDeployment not found")
		return false, nil
	}
	...
	if !(version.GreaterThanOrEqual(versionCoreDNSMin) && version.LessThan(versionCoreDNSMax)) {
		c.logger.Info(`CoreDNS version is not supported, must satisfy ">= %s, < %s", got %q`, versionCoreDNSMin, versionCoreDNSMax, version)

		return false, fmt.Errorf("unsupported CoreDNS version %q", version)
	}

	c.logger.Info("CoreDNS %q has been detected", version)

	return true, nil
}

func (c *Client) kubeDNSMatch(ctx context.Context) (bool, error) {
	c.logger.Info("Checking if KubeDNS is installed", "Namespace", metav1.NamespaceSystem)

	_, err := c.kubeClient.xdep.get(ctx, metav1.NamespaceSystem, "kube-dns")
	if kerrors.IsNotFound(err) {
		c.logger.Info("KubeDNS deployment not found")
		return false, nil
	}
	...
	c.logger.Info("KubeDNS has been detected")
...
func (c *Client) ConfigureCoreDNS(ctx context.Context, dnsServiceNamespace, dnsServiceName string, dnsServicePort int32) error {
	...
	if !changed {
		c.logger.Info("CoreDNS ConfigMap %q in namespace %q has already been patched", configMap.Name, configMap.Namespace)

		return nil
	}

	if _, err = c.kubeClient.xmap.update(ctx, configMap.Namespace, configMap); err != nil {
		return err
	}

	c.logger.Info("CoreDNS ConfigMap  has successfully been patched",
...
func (c *Client) patchCoreDNSConfig(ctx context.Context, deployment *appsv1.Deployment, dnsServiceIP string, dnsServicePort int32) (*corev1.ConfigMap, bool, error) {
	...
	if err == nil {
		corefile, changed := addStubDomain(
			customConfigMap.Data["dream.mesh.server"],
			blockHeader,
			blockTrailer,
			dnsServiceIP,
			dnsServicePort,
		...
		customConfigMap.Data["dream.mesh.server"] = corefile
...
func (c *Client) ConfigureKubeDNS(ctx context.Context, dnsServiceNamespace, dnsServiceName string, dnsServicePort int32) error {
	dnsDeployment, err := c.kubeClient.xdep.get(ctx, metav1.NamespaceSystem, "kube-dns")
	if err != nil {
		return err
	}
	...
	c.logger.Info("Found ClusterIP ",
		"Service", "coredns", "namespace", metav1.NamespaceSystem, "IP", dnsServiceIP)
...
func (c *Client) patchKubeDNSConfig(ctx context.Context, deployment *appsv1.Deployment, dnsServiceIP string, dnsServicePort int32) error {
	...
	// Add our stubDomain.
	stubDomains["dream.mesh"] = []string{fmt.Sprintf("%s:%d", dnsServiceIP, dnsServicePort)}
...
func (c *Client) unpatchCoreDNSConfig(ctx context.Context, deployment *appsv1.Deployment) (*corev1.ConfigMap, error) {
	...
	// See https://docs.microsoft.com/en-us/azure/aks/coredns-custom
	if err == nil {
		delete(coreDNSConfigMap.Data, "dream.mesh.server")
...
func (c *Client) getOrCreateConfigMap(ctx context.Context, deployment *appsv1.Deployment, name string) (*corev1.ConfigMap, error) {
	...
	configMap, err := c.kubeClient.xmap.get(ctx, deployment.Namespace, volume.Name)
	...
	if kerrors.IsNotFound(err) && volume.Optional != nil && *volume.Optional {
		...
		configMap, err = c.kubeClient.xmap.create(ctx, deployment.Namespace, configMap)
	}
...
func (c *Client) restartPods(ctx context.Context, deployment *appsv1.Deployment) error {
	c.logger.Info("Restarting pods", "Deployment", deployment.Name)
	...
	annotations["dream-mesh-hash"] = uuid.New().String()
	deployment.Spec.Template.Annotations = annotations

	_, err := c.kubeClient.xdep.update(ctx, deployment.Namespace, deployment)
...
func (c *Client) getServiceIP(ctx context.Context, namespace, name string) (string, error) {
	...
	operation := func() error {
		service, err := c.kubeClient.xsvc.get(ctx, namespace, name)
		if err != nil {
